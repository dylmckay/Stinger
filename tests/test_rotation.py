...
async def _seed(factory):
    ...
    async with factory() as s:
        ...
        ep = Endpoint(application_id=a.id, url="https://x.test/h", secret=old, status="enabled")
...
async def test_rotation_sets_window_and_dual_signs(session_factory):
    ...
    assert ep.secret == new and ep.previous_secret == old
    # within the window, a delivery verifies under both secrets
    headers = signing.sign("p", message_id="m", secret=ep.secret, previous_secret=ep.previous_secret)
...
async def test_rotation_is_tenant_scoped(session_factory):
    ...
    async with session_factory() as s:
        assert (await s.get(Endpoint, ep_id)).secret == old   # untouched